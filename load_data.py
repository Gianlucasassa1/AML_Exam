...
def tokenize_description(item, description_titles):
    ...
    return ' '.join(concat_descr.split(' ')[:75]) #Limit the number of description words to 77 because of clip.tokenize limits. 
...
def build_splits_clip_disentangle(opt):
    ...
    load_our_descriptions = json.load(open("./data/clip/s301212_s305931_labeled.json", "r"))
    our_descriptions = create_dict(load_our_descriptions)

    load_all_descriptions = json.load(open("./data/clip/PACS_descriptions.json", "r"))
    all_descriptions = {}
    for key,value in load_all_descriptions.items():
        if len(value)==9:
          all_descriptions[key] = tokenize_description(value, description_titles)
 
    descriptions = dict()
    descriptions.update(all_descriptions)
    descriptions.update(our_descriptions)
    ...
    # Build splits - we train and validate on the target domain 
    target_val_split_length = target_total_examples * 0.2 # 20% of the training split used for validation and test
