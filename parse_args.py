...
def parse_arguments():
    ...
    parser.add_argument('--weights', type=json.loads, default="[12, 0.01, 0.01, 0.8, 0.2]", help="if set, the experiment will run on different weights than the default value. Format: --weights=[weight1, weight2, weight3, alpha, clip_weight]") #weight1, weight2, weight3, alpha, clip
    ...
    parser.add_argument('--reconstructor_type', type=str, default='linear')
    ...
    opt['output_path'] = f'{opt["output_path"]}/record/{opt["experiment"]}_{opt["target_domain"]}_{opt["encoder_type"]}'
